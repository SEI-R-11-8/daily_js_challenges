...
function range(start, finish) {
  if (start > finish) return 'first argument must be less than the second'
...
function reduceArray(arr, cb, initialAcc) {
  let acc = initialAcc
  arr.forEach(function (el, i) {
    acc = cb(acc, el, i)
  })
...
function intersection(arr1, arr2) {
  ...
  let _arr2 = [...arr2]
  arr1.forEach((val) => {
    let i = _a2.indexOf(val)
    if (i > -1) result.push(_arr2.splice(i, 1)[0])
  })
  return result
}
...
function balancedBrackets(str) {
  ...
  let stack = []
  for (let i = 0; i < str.length; i++) {
    let braces = str.charAt(i)
    if ('([{'.includes(braces)) {
      stack.push(braces)
    } else {
      if (!'() {} []'.includes(stack.pop() + braces)) return false
    }
  }
  return true
}
