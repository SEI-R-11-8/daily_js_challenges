...
function computeRemainder(n1, n2) {
  return n2 === 0 ? Infinity : n1 - Math.floor(n1 / n2) * n2;
}
/*-----------------------------------------------------------------
Challenge: 06-range
...
function reverseUpcaseString(string) {
  ...
  return (
    ...
    reverseUpcaseString(string.substring(0, string.length - 1))
  ).toUpperCase();
}
/*-----------------------------------------------------------------
...
function mapArray(arr, cb) {
  // We literally just went over this in class, haha
  let result = [];
...
function reduceArray(arr, acc, value) {
  ...
  for (let i = 0; i < arr.length; i++) {
    resultAcc = acc(resultAcc, arr[i], i, arr);
  }
...
function flatten(arr) {
  ...
  return arr.reduce((accumulator, element) => {
    if (Array.isArray(element)) {
      accumulator.push(...flatten(element));
    } else {
...
function intersection(arr1, arr2) {
  ...
  arr1.forEach((element) => {
    commonValues[element.toString()] = true;
  });
  arr2.forEach((element) => {
    if (commonValues[element.toString()]) {
      result.push(element);
    }
  });
...
function balancedBrackets(string) {
  ...
  for (let i = 0; i < string.length; i++) {
    ...
    let bracketOpposite = bracketOpposites[brackets[brackets.length - 1]];

    if (bracketOpposites[currentBracket]) {
      brackets.push(currentBracket);
    } else if (currentBracket === bracketOpposite) {
      brackets.pop();
    }
  }
...
function isWinningTicket(arr) {
  for (let i = 0; i < arr.length; i++) {
    ...
    let currentCode = currentPair[1];

    let hasMatchingCode = false;
    for (let j = 0; j < currentKey.length; j++) {
      if (currentKey.charCodeAt(j) === currentCode) {
...
function totalTaskTime(arr, n) {
  ...
  */
  ...
  while (arr.some((task) => task > 0)) {
    for (let i = 0, j = n; i < arr.length && j > 0; i++) {
      if (arr[i] > 0) {
        j--;
        arr[i]--;
      }
