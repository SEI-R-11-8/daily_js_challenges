...
function isPalindrome (string) {
  ...
  myString = lowerString.replaceAll(' ', '')
  let splitString = myString.split('')
  let joinString = splitString.join('')
  let revString = splitString.reverse()
  let reverseString = revString.join('')
  if (reverseString === joinString) {
    return true
  } else {
    return false
  }
...
function mumble (string) {
  let tempArr = []
  string.split('').forEach((e, index) => {
    nums = []
    for(let i = 0; i <= index; i++) {
      nums.push(e)
    }
    tempArr.push(nums.join(''))
  })
  myStr = tempArr.join('-')
  return myStr
}
